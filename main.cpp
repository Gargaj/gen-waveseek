		...
		case WM_PAINT:
			{
				PAINTSTRUCT ps = {0};
				BeginPaint(hWnd, &ps);
				PaintWaveform(ps.hdc, ps.rcPaint);
				EndPaint(hWnd, &ps);
			} break;
		case WM_LBUTTONUP:
			{
		...
		case WM_MOUSEMOVE:
			{
				...
				if (nLengthInMS != -1)
				{
					RECT rc;
					GetClientRect(hWnd,&rc);
					unsigned int sec = MulDiv(xPos,nLengthInMS,(rc.right - rc.left)) / 1000;
					...
					char coords[32] = {0};
					StringCchPrintf(coords, 32, "%d:%02d", sec / 60, sec % 60);
					ti.lpszText = coords;
...
extern "C" {
	...
	__declspec(dllexport) int winampUninstallPlugin(HINSTANCE hDllInst, HWND hwndDlg, int param){
		...
		if(MessageBox(hwndDlg, "Do you also want to remove the saved settings for this plug-in?",
